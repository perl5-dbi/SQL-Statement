...
##
    ...
    else
    {
        ...
        return $self->do_err(
                           "Command '$com' not recognized or not supported!");
    }
}
...
sub INSERT
{
    ...
    my ( $table_name, $val_str ) =
      $str =~ /^INSERT\s+(.+?)\s+VALUES\s+\((.+?)\)$/i;
    if ( $table_name and $table_name =~ /[()]/ )
    {
        ( $table_name, $col_str, $val_str ) =
          $str =~ /^INSERT\s+(.+?)\s+\((.+?)\)\s+VALUES\s+\((.+?)\)$/i;
    }
        ...
        if ($constraints)
        {
            ...
            for my $constr (@c)
            {
                if ( $constr =~ /^\s*(UNIQUE|NOT_NULL|PRIMARY_KEY)\s*$/i )
                {
                    ...
                    $constr =~ s/_/ /g;
                    push @{ $self->{struct}->{column_defs}->{"$name"}
                          ->{constraints} }, $constr;
                ...
                {
                    return $self->do_err(
                                     "Unknown column constraint: '$constr'!");
                }
            }
        }
        $type = uc $type;
...
sub SELECT_LIST
{
    ...
    for my $col (@col_list)
    {
        ...
        if ( $col =~ /^(\S+)\.\*$/ )
        {
            ...
            $table = $is_table_alias{"\L$table"}
              if $is_table_alias{"\L$table"};
        ...
        else
        {
            ...
            if ( !$newcol )
            {
                ...
                if (
                     ref($func_obj) =~ /::Function$/
                     or (     ref($func_obj) eq 'HASH'
                          and $func_obj->{type}
                          and $func_obj->{type} eq 'function' )
                   )
                {
                    ...
                    $self->{struct}->{col_obj}->{$newcol} =
                      SQL::Statement::Util::Column->new( $newcol, [],
                                                       $newalias, $func_obj );
                }
...
sub get_btwn
{
    ...
    while ( $str =~ /^(.+?)\b(NOT\s+)?BETWEEN (.+)$/i )
    {
        ...
        #
        my $col =
          ( $front =~ s/^(.+\b(AND|NOT|OR))\b(.+)$/$1/i ) ? $3 : $front;
        $front = ''
          if ( $col eq $front );
...
sub get_in
{
    ...
    while ( $str =~ /^(.+?)\b(NOT\s+)?IN \((.+)$/i )
    {
        ...
        return $self->do_err(
                          "Unmatched right parentheses during IN processing!")
        ...
        return $self->do_err(
                           "Unmatched left parentheses during IN processing!")
          if ( $in_inside_parens > 0 );
...
sub non_parens_search
{
    ...
    if ( $str =~ /\G(.*?)\s+(AND|OR)\s+(.*)$/igcs )
    {
        ...
        {
            $pred1 =
              $self->non_parens_search( $$predicates[$1], $predicates );
        }
        else
        {
...
sub ROW_VALUE
{
         ...
         and $user_func_name !~ /(TRIM|SUBSTRING)/i )
    {
        ...
        if ( $self->{opts}->{function_names}->{$name} )
        {
            ...
            return
              SQL::Statement::Util::Function->new( $name, $usr_sub->{value},
                                                   \@final_args, )
              if $usr_sub;
    ...
    if ( $str =~ /^SUBSTRING \((.+?) FROM (.+)\)\s*$/i )
    {
        ...
        return $self->do_err(
          ...
          if $start->{type} eq 'string'
              or (     $start->{length}
                   and $start->{length}->{type} eq 'string' );
...
sub COLUMN_NAME
{
    ...
    {
        undef $user_func
          unless $self->{opts}->{function_names}->{ uc $user_func };
    }
    if ( !$user_func )
    {
        return undef
