...
a subclass to parse SQL statements and allow you to examine the structure
...
a parser object, how to use it to parse and examine SQL statements.
See L<SQL::Statement> for other usages of the module.

=head1 B<Creating a parser object>

The parser object only needs to be created once per script. It can then be
reused to parse any number of SQL statements. The basic creation of a parser
is this:
...
but doesn't attempt to execute the SQL unless you explicitly call execute().
...
to be inlcuded in the limit clause).
...
directly after successful parsing a statement from the given C<where_clause>,
if any.
